...
fn trailhead_score(map: &Map<u8>, start: Point2D<usize>) -> usize {
    ...
    for i in 1..=9 {
        starts = starts.into_iter().flat_map(|pt| map.four_neighborhood(pt)).filter(|pt| *map.at(*pt) == i).collect();
    }
    starts.len()
}
...
pub fn run(input: &str) -> Result<usize> {
    ...
    Ok(trailheads.into_iter().map(|start| trailhead_score(&map, start)).sum())
}
